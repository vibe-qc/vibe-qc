"""compute-cluster lattice scan: fcc Cu 2x2x2, T = 0.005 Ha (sane branch).

Usage: run_cu.py a1 a2 a3 ...  (lattice constants in Angstrom)
Fail-closed points (the compressed-volume over-polarized basin) print
FAILED and exit 1, but the parabola minimum is still reported.
"""
...
def run_point(a_angstrom: float):
    ...
    for i in range(2):
        for j in range(2):
            for k in range(2):
                atoms.append(i * prim[0] + j * prim[1] + k * prim[2])
                zs.append(29)
...
def main() -> None:
    ...
    for a in a_list:
        try:
            ...
            print(f"{a:8.4f}  FAILED(fail-closed): {str(exc)[:50]}",
                  flush=True)
            failed += 1
            continue
    ...
    print(f"{failed} point(s) fail-closed (expected at compressed a)", flush=True)
