#!/usr/bin/env python3
"""Exploratory far-geometry CO/MgO(001) periodic SKALA calculation.

This is the independent companion to ``co_mgo_near.py`` with Mg-C fixed at
an illustrative separated distance of 6.0 Angstrom. All other geometry,
basis, and numerical settings are identical.

The near-minus-far energy is only a vacuum-padded 3D, high-coverage, unrelaxed
STO-3G surrogate. It is not a vacuum-free 2D slab or a converged adsorption
energy, and periodic SKALA dispersion is unavailable.
"""
...
def reserve_output_stem(requested: Path | None) -> Path:
    ...
    output_dir = root / "co-mgo-far-results"
    output_dir.mkdir(exist_ok=False)
    probe = output_dir / ".write-probe"
    probe.write_text("ok\n", encoding="utf-8")
    probe.unlink()
    return output_dir / "co-mgo-far"
...
def main() -> None:
    ...
    if not result.converged or not math.isfinite(result.energy):
        raise RuntimeError("far CO/MgO SKALA did not converge to a finite energy")

    print(f"output stem: {output}")
    print(f"Mg-C distance = {MG_C_DISTANCE_ANGSTROM:.3f} Angstrom")
    print(f"E_far/cell    = {result.energy:.12f} hartree")
