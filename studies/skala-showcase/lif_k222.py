#!/usr/bin/env python3
"""Experimental LiF rocksalt SKALA-1.1 2x2x2 GDF calculation.

Run only after ``lif_gamma.py`` converges to a finite, plausible result. The
mesh is vibe-qc's Gamma-centred convention, not the shifted even Monkhorst-Pack
mesh used by some other programs. Periodic SKALA remains experimental.
"""
...
def reserve_output_stem(requested: Path | None) -> Path:
    ...
    output_dir = root / "lif-k222-results"
    output_dir.mkdir(exist_ok=False)
    probe = output_dir / ".write-probe"
    probe.write_text("ok\n", encoding="utf-8")
    probe.unlink()
    return output_dir / "lif-k222"
...
def main() -> None:
    ...
    result = vq.run_periodic_job(
        ...
        # Full-k RKS has no exact population sidecar. Do not request the
        # molecular Gamma-block proxy for this multi-k calculation.
    ...
    if not result.converged or not math.isfinite(result.energy):
        raise RuntimeError("LiF k222 SKALA did not converge to a finite energy")

    print(f"output stem: {output}")
    print(f"E(SKALA-1.1, k222)/cell = {result.energy:.12f} hartree")
    print("k-point convention: Gamma-centred 2x2x2")
