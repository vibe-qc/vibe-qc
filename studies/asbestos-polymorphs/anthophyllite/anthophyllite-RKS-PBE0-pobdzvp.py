"""anthophyllite — RKS-PBE0 / pob-DZVP-rev2 / Γ via GDF.

Mg₇Si₈O₂₂(OH)₂, Pnma (orthorhombic). Walitzi 1965 / Sueno 1972. Mg endmember.
...
* PBE0 is the canonical solid-state hybrid (α=0.25). Sister script
  ``anthophyllite-RKS-PW1PW-pobdzvp.py`` uses PW1PW (α=0.20, PWGGA
...
from _minerals import anthophyllite_Mg

import vibeqc as vq

OUTPUT_STEM = Path(__file__).resolve().with_suffix("")

system, label = anthophyllite_Mg()
basis = vq.BasisSet(system.unit_cell_molecule(), "pob-dzvp-rev2")

print(f"anthophyllite: {label}, nbf={basis.nbasis}, "
