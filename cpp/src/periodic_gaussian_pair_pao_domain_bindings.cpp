// Included by bindings.cpp. Actual-HF diagnostics with a native AO atom map.
#include <pybind11/pybind11.h>
#include "vibeqc/periodic_gaussian_pair_pao_domain.hpp"
#ifndef VIBEQC_BINDINGS_HAS_PY_ALIAS
namespace py=pybind11;
#endif

void bind_periodic_gaussian_pair_pao_domain(py::module_& m) {
    using namespace vibeqc;
    using Inventory=PeriodicCorrelationPairPAODomainInventory;
    using Caps=PeriodicGaussianPairPAODomainCaps;
    using Plan=PeriodicGaussianPairPAODomainMemoryPlan;
    using Result=PeriodicGaussianPairPAODomain;
    using Endpoint=PeriodicGaussianOccupiedPAODomain;
    using Topology=PeriodicCorrelationTranslationPairTopology;
    py::class_<Caps>(m,"_PeriodicGaussianPairPAODomainCaps").def(py::init<>())
        .def_readwrite("pair_union",&Caps::pair_union)
        .def_readwrite("maximum_owned_numerical_bytes",&Caps::maximum_owned_numerical_bytes)
        .def_readwrite("maximum_worker_bytes",&Caps::maximum_worker_bytes)
        .def_readwrite("maximum_node_bytes",&Caps::maximum_node_bytes)
        .def_readwrite("maximum_work_units",&Caps::maximum_work_units);
    auto plan=py::class_<Plan>(m,"_PeriodicGaussianPairPAODomainMemoryPlan");
#define GPAIR_PLAN(field) plan.def_readonly(#field,&Plan::field)
    GPAIR_PLAN(pair_union); GPAIR_PLAN(atom_mapping_bytes); GPAIR_PLAN(borrowed_gaussian_numerical_bytes);
    GPAIR_PLAN(additional_control_storage_bytes); GPAIR_PLAN(peak_owned_numerical_bytes);
    GPAIR_PLAN(worker_bytes); GPAIR_PLAN(required_node_memory_bytes);
    GPAIR_PLAN(physical_input_validation_work_units); GPAIR_PLAN(mapping_work_units); GPAIR_PLAN(work_units);
#undef GPAIR_PLAN
    py::class_<Result>(m,"_PeriodicGaussianPairPAODomain")
        .def_property_readonly("domain",&Result::domain,py::return_value_policy::reference_internal)
        .def_property_readonly("memory",[](const Result& r) { return r.memory(); })
        .def_property_readonly("context",&Result::context_handle)
        .def_property_readonly("matched_finite_gaussian_hf_recipe",&Result::matched_finite_gaussian_hf_recipe)
        .def_property_readonly("production_dlpno",&Result::production_dlpno)
        .def_property_readonly("infinite_source_accuracy_certified",&Result::infinite_source_accuracy_certified)
        .def_property_readonly("hf_reference_source_identity_sha256",&Result::hf_reference_source_identity_sha256)
        .def_property_readonly("localization_identity_sha256",&Result::localization_identity_sha256)
        .def_property_readonly("identity_sha256",&Result::identity_sha256);
    const auto tiny=[](const PeriodicGaussianRHFResult& hf,const PeriodicCorrelationAdmittedReference& ref,
                      const Topology& topology) {
        if (!ref.state_handle() || ref.state().n_kpoints()>8 || ref.state().n_basis()>12
            || ref.state().n_correlated_occupied()>4 || hf.plan().atom_count>8 || topology.row_count()>512)
            throw std::length_error("Gaussian pair PAO domain exceeds tiny diagnostic dimensions");
    };
    const auto bounded=[](const Plan& p) {
        if (p.peak_owned_numerical_bytes>(8U<<20) || p.required_node_memory_bytes>(128U<<20)
            || p.work_units>100000000000000ULL)
            throw std::length_error("Gaussian pair PAO domain exceeds tiny diagnostic memory/work limits");
    };
    m.def("_plan_periodic_gaussian_pair_pao_domain",[tiny,bounded](
        const PeriodicGaussianRHFResult& hf,const PeriodicCorrelationAdmittedReference& ref,
        const Topology& topology,std::uint64_t row,const Endpoint& home,const Endpoint& partner,
        Inventory inventory,Caps caps) {
        tiny(hf,ref,topology);
        const auto p=plan_periodic_gaussian_pair_pao_domain(hf,ref,topology,row,home,partner,inventory,caps);
        bounded(p); return p;
    },py::arg("hf"),py::arg("reference"),py::arg("topology"),py::arg("row_index"),py::arg("home"),
      py::arg("partner"),py::arg("inventory"),py::arg("caps"));
    m.def("_make_periodic_gaussian_pair_pao_domain",[tiny,bounded](
        const PeriodicGaussianRHFResult& hf,const PeriodicCorrelationAdmittedReference& ref,
        const BasisSet& ao,const BasisSet& auxiliary,const PeriodicSystem& system,
        const Topology& topology,std::uint64_t row,const Endpoint& home,const Endpoint& partner,
        Inventory inventory,Caps caps) {
        tiny(hf,ref,topology);
        bounded(plan_periodic_gaussian_pair_pao_domain(hf,ref,topology,row,home,partner,inventory,caps));
        // Exact argument owners stay pinned; no GIL release, callback or
        // caller-provided numerical array can substitute for the native map.
        return make_periodic_gaussian_pair_pao_domain(hf,ref,ao,auxiliary,system,topology,row,
            home,partner,inventory,caps);
    },py::arg("hf"),py::arg("reference"),py::arg("ao_basis"),py::arg("auxiliary_basis"),py::arg("system"),
      py::arg("topology"),py::arg("row_index"),py::arg("home"),py::arg("partner"),py::arg("inventory"),py::arg("caps"));
}
