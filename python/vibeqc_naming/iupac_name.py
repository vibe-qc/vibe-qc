...
def name_from_atoms_with_lattice(
    ...
    **solid_options,
    ...
    """Name a periodic solid, or a molecule when no periodic axes are given.

    Lattice and coordinates are in angstrom. A lattice without explicit PBC
    means three-dimensional periodicity; vacuum alone does not establish a
    slab. Pass ``pbc=(True, True, False)`` for a 2D slab. Additional options
    are documented by :func:`name_solid_from_atoms`.
    """
    if pbc is not None:
        from .solids import _periodic_flags

        pbc = _periodic_flags(pbc)
    if lattice_vectors is None or (pbc is not None and not any(pbc)):
        if solid_options:
            raise ValueError("solid options require a periodic lattice")
        if lattice_vectors is None and pbc is not None and any(pbc):
            raise ValueError("periodic boundary conditions require lattice_vectors")
        return name_from_atoms_detailed(atoms, prefer_trivial=prefer_trivial)

    return name_solid_from_atoms(
        atoms, lattice_vectors, pbc=(True, True, True) if pbc is None else pbc,
        prefer_trivial=prefer_trivial, **solid_options,
    )


def name_periodic_system(
    graph: MolecularGraph,
    lattice_vectors: Optional[list[tuple[float, float, float]]] = None,
    *,
    prefer_trivial: bool = True,
    ...
    **solid_options,
    ...
    """Name the unit-cell composition without treating periodic bonds as molecules."""
    ...
    return name_from_atoms_with_lattice(
        ...
        prefer_trivial=prefer_trivial, pbc=pbc, **solid_options,
    )
...
def formula_to_name_detailed(formula: str) -> NamedResult:
    ...
    if _canonical_formula(formula) in _TRIVIAL_NAMES:
        return NamedResult(
...
def _name_looks_garbage(name: str) -> bool:
    """Return True if *name* looks like a malformed organic systematic name.

    Signs: trailing hyphen, bare 'methan' root without a recognised suffix,
    repeated 'methan' fragments, or a name consisting only of substituent prefixes.
    """
    if not name:
        return True
    if name.endswith('-'):
        return True
    # Bare 'methan' without a proper alkane/functional-group suffix
    if 'methan' in name:
        suffixes = ('methane', 'methanol', 'methanal', 'methanoic', 'methanamine',
                     'methanamide', 'methanenitrile', 'methanethiol')
        if not any(s in name for s in suffixes):
            return True
    # Repeated 'methan' fragments (sign of recursive substituent naming)
    if name.count('methan') >= 3:
        return True
    # Name is only substituent prefixes with no parent chain
    parts = name.replace(',', ' ').split()
    has_parent = any(p.endswith(('ane', 'ene', 'yne', 'ol', 'al', 'one', 'oic acid',
                                  'amine', 'nitrile', 'amide', 'oate')) for p in parts)
    if not has_parent and any('-' in p for p in parts):
        return True
    return False
...
def _name_from_graph(
    ...
    *,
    ...
    if prefer_trivial:
        ...
        if canonical in _TRIVIAL_NAMES:
            return NamedResult(
                name=_TRIVIAL_NAMES[canonical],
                source=NamingSource.TRIVIAL_IUPAC,
                confidence=Confidence.HIGH,
                formula=formula,
                is_trivial=True,
        ...
        if key in _STRUCTURAL_NAMES:
            # Formula + ring count + atom count alone does not uniquely
            # identify a molecule: C6H12 with one ring could be
            # cyclohexane, methylcyclopentane, or a different isomer.
            # This heuristic is therefore MEDIUM confidence, not HIGH.
            return NamedResult(
    ...
    # Check for periodic 2D surfaces with or without adsorbed molecules.
    # This runs before coordination naming because metal oxides are
    # not coordination complexes in the IUPAC sense.
    if graph.n_atoms >= 10:  # cheap gate: surfaces need many atoms
        from .surface import name_slab_system

        slab_result = name_slab_system(graph, None, fast_mode=True)
        if slab_result.is_trivial or slab_result.confidence in (
            Confidence.HIGH,
            Confidence.MEDIUM,
        ):
            return slab_result
    ...
    # ── 3. Coordination complex detection (Red Book IR-7, IR-9) ───────────
    ...
    # ── 3a. Multi-component check before organic naming ────────────
    components = graph.connected_components()
    if len(components) > 1:
        names = []
        from collections import Counter
        for comp in components:
            if len(comp) <= 2:  # skip tiny fragments
                continue
            sub = graph.subgraph(comp)
            sub_result = _name_from_graph(sub, prefer_trivial=prefer_trivial, with_external=False)
            if sub_result.name:
                names.append(sub_result.name)
        if len(names) >= 2:
            cnt = Counter(names)
            parts = []
            for n, c in sorted(cnt.items()):
                parts.append(n if c == 1 else f"{c} x {n}")
            return NamedResult(
                name=" + ".join(parts),
                source=NamingSource.INORGANIC_COMPOSITIONAL,
                confidence=Confidence.HIGH,
    ...
    if graph.is_organic:
        ...
        if organic_result:
            ...
            _name = organic_result.name if isinstance(organic_result, OrganicResult) else organic_result
            if _name_looks_garbage(_name):
                conf = Confidence.LOW
            return NamedResult(
                name=_name,
                source=source,
    ...
    # ── 4. Multi-component fallback (already checked above) ──────────
    components = graph.connected_components()
    if len(components) > 1:
        names = []
        from collections import Counter
        for comp in components:
            if len(comp) <= 1:  # skip isolated H atoms etc
                continue
            sub = graph.subgraph(comp)
            sub_result = _name_from_graph(sub, prefer_trivial=prefer_trivial, with_external=False)
            if sub_result.name:
    ...
    # ── 5. Inorganic compositional naming (Red Book IR-4, IR-5) ───────────
    ...
    # ── 6. External backends (optional) ────────────────────────────────────
    if with_external:
        external_result = _try_external(graph)
        if not external_result:
            # Try SMILES-based lookup as a more accurate fallback
            try:
                from .smiles import graph_to_smiles
                from .external import name_from_smiles_external
                smi = graph_to_smiles(graph)
                if smi:
                    external_result = name_from_smiles_external(smi)
            except Exception:
                pass
        if external_result:
    ...
    # ── 7. Ultimate fallback: compositional from formula ───────────────────
...
def _electronegativity_rank(symbol: str) -> int:
    """Return ordering rank: higher number = more electropositive."""
    try:
...
# Hill formula aliases: traditional formula order (NH3, HCl) vs Hill order (H3N, ClH).
...
def _element_name(symbol: str) -> str:
    ...
    return _EL.get(symbol, symbol.lower())


def _build_compositional_name(counts: dict[str, int]) -> str:
    """Compositional name from element count dict.

    Per Red Book IR-4: elements ordered by electronegativity (more
    electropositive first). The most electronegative element gets
    the -ide suffix in binary compounds.
    """
    if not counts:
        return ""
    ...
    # Sort: more electropositive first (IUPAC convention)
    sorted_elems = sorted(
        counts.items(),
    ...
    for i, (sym, count) in enumerate(sorted_elems):
        prefix = _GREEK_PREFIX.get(count, str(count))
        # Last element gets -ide suffix (Red Book IR-4.3)
...
def name_from_qvf(qvf_path: str, **solid_options) -> NamedResult:
    ...
    from ._qvf_read import QVFReadError, QVFReader
    ...
    if parts:
        result = result._replace(formula="".join(parts))
