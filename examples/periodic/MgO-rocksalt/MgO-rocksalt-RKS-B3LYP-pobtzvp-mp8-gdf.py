"""MgO rocksalt primitive cell -- RKS-B3LYP / pob-TZVP / 8x8x8 native GDF target.

Standalone vibe-qc input for side-by-side CRYSTAL14 parity.

CRYSTAL14 partner:
    MgO-rocksalt-RKS-B3LYP-pobtzvp-mp8-crystal14.d12
...
This vibe-qc input builds that same primitive cell explicitly. The
native vibe-qc Gamma-only GDF backend is available, but the 8x8x8 multi-k
GDF/FFTDF backend is still pending. Until that lands, this target input
stops with ``NotImplementedError`` instead of importing PySCF.

Run:
    .venv/bin/python examples/periodic/MgO-rocksalt/MgO-rocksalt-RKS-B3LYP-pobtzvp-mp8-gdf.py
"""
...
FUNCTIONAL = "b3lyp"  # = VWN5 flavor (ORCA definition) == CRYSTAL14 B3LYP keyword
...
def make_options() -> vq.PeriodicKSOptions:
    opts = vq.PeriodicKSOptions()
    opts.functional = FUNCTIONAL
    # CRYSTAL14 parity controls:
    #   FMIXING 30   -> 30% KS-matrix mixing
...
if __name__ == "__main__":
    ...
    with vq.perf_log(OUT_STEM.with_suffix(".perf")):
        plog.banner("MgO rocksalt / primitive / RKS-B3LYP / pob-TZVP / native GDF target")
        ...
        result = vq.run_krks_periodic_gdf(
            system, basis, kmesh,
            options=opts,
        ...
        plog.info(f"  E_xc           = {result.e_xc:.10f} Ha")
