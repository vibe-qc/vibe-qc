"""β-SiC (zincblende) — RHF on pob-tzvp (Γ-only).
...
Compound metadata:
    formula:        β-SiC (zincblende)
    space group:    F-43m
    crystal system: cubic
    a, b, c (Å):    4.3580, 4.3580, 4.3580
    α, β, γ (deg):  90.00, 90.00, 90.00
    n_atoms in cell: 8
    multiplicity:   1
    structure src:  PT2013 Ref. 52
...
def main() -> None:
    lattice = np.array([
        [8.2354258583, 0.0000000000, 0.0000000000],
        [0.0000000000, 8.2354258583, 0.0000000000],
        [0.0000000000, 0.0000000000, 8.2354258583],
    ])  # bohr

    unit_cell = [
        vq.Atom(14, [  0.0000000000,   0.0000000000,   0.0000000000]),  # Si frac=(0.000,0.000,0.000)
        vq.Atom(14, [  4.1177129292,   4.1177129292,   0.0000000000]),  # Si frac=(0.500,0.500,0.000)
        vq.Atom(14, [  4.1177129292,   0.0000000000,   4.1177129292]),  # Si frac=(0.500,0.000,0.500)
        vq.Atom(14, [  0.0000000000,   4.1177129292,   4.1177129292]),  # Si frac=(0.000,0.500,0.500)
        vq.Atom( 6, [  2.0588564646,   2.0588564646,   2.0588564646]),  # C frac=(0.250,0.250,0.250)
        vq.Atom( 6, [  6.1765693937,   6.1765693937,   2.0588564646]),  # C frac=(0.750,0.750,0.250)
        vq.Atom( 6, [  6.1765693937,   2.0588564646,   6.1765693937]),  # C frac=(0.750,0.250,0.750)
        vq.Atom( 6, [  2.0588564646,   6.1765693937,   6.1765693937]),  # C frac=(0.250,0.750,0.750)
    ]
    ...
    print(f"  cell:       {lattice[0][0]:.4f} bohr (= 4.3580 Å)")
