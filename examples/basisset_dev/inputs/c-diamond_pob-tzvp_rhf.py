"""C (diamond) — RHF on pob-tzvp (Γ-only).
...
Compound metadata:
    ...
    a, b, c (Å):    3.5670, 3.5670, 3.5670
    α, β, γ (deg):  90.00, 90.00, 90.00
    n_atoms in cell: 8
    multiplicity:   1
    structure src:  PT2013 Ref. 24 (BSE-recommended experimental value)
...
def main() -> None:
    lattice = np.array([
        [6.7406526013, 0.0000000000, 0.0000000000],
        [0.0000000000, 6.7406526013, 0.0000000000],
        [0.0000000000, 0.0000000000, 6.7406526013],
    ])  # bohr

    unit_cell = [
        vq.Atom( 6, [  0.0000000000,   0.0000000000,   0.0000000000]),  # C frac=(0.000,0.000,0.000)
        vq.Atom( 6, [  3.3703263007,   3.3703263007,   0.0000000000]),  # C frac=(0.500,0.500,0.000)
        vq.Atom( 6, [  3.3703263007,   0.0000000000,   3.3703263007]),  # C frac=(0.500,0.000,0.500)
        vq.Atom( 6, [  0.0000000000,   3.3703263007,   3.3703263007]),  # C frac=(0.000,0.500,0.500)
        vq.Atom( 6, [  1.6851631503,   1.6851631503,   1.6851631503]),  # C frac=(0.250,0.250,0.250)
        vq.Atom( 6, [  5.0554894510,   5.0554894510,   1.6851631503]),  # C frac=(0.750,0.750,0.250)
        vq.Atom( 6, [  5.0554894510,   1.6851631503,   5.0554894510]),  # C frac=(0.750,0.250,0.750)
        vq.Atom( 6, [  1.6851631503,   5.0554894510,   5.0554894510]),  # C frac=(0.250,0.750,0.750)
    ]
    ...
    print(f"  cell:       {lattice[0][0]:.4f} bohr (= 3.5670 Å)")
