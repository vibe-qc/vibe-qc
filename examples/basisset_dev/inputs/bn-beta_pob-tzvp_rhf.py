"""β-BN (zincblende) — RHF on pob-tzvp (Γ-only).
...
Compound metadata:
    formula:        β-BN (zincblende)
    space group:    F-43m
    crystal system: cubic
    a, b, c (Å):    3.6250, 3.6250, 3.6250
    α, β, γ (deg):  90.00, 90.00, 90.00
    n_atoms in cell: 8
    multiplicity:   1
    structure src:  PT2013 Ref. 27
...
def main() -> None:
    lattice = np.array([
        [6.8502567087, 0.0000000000, 0.0000000000],
        [0.0000000000, 6.8502567087, 0.0000000000],
        [0.0000000000, 0.0000000000, 6.8502567087],
    ])  # bohr

    unit_cell = [
        vq.Atom( 5, [  0.0000000000,   0.0000000000,   0.0000000000]),  # B frac=(0.000,0.000,0.000)
        vq.Atom( 5, [  3.4251283543,   3.4251283543,   0.0000000000]),  # B frac=(0.500,0.500,0.000)
        vq.Atom( 5, [  3.4251283543,   0.0000000000,   3.4251283543]),  # B frac=(0.500,0.000,0.500)
        vq.Atom( 5, [  0.0000000000,   3.4251283543,   3.4251283543]),  # B frac=(0.000,0.500,0.500)
        vq.Atom( 7, [  1.7125641772,   1.7125641772,   1.7125641772]),  # N frac=(0.250,0.250,0.250)
        vq.Atom( 7, [  5.1376925315,   5.1376925315,   1.7125641772]),  # N frac=(0.750,0.750,0.250)
        vq.Atom( 7, [  5.1376925315,   1.7125641772,   5.1376925315]),  # N frac=(0.750,0.250,0.750)
        vq.Atom( 7, [  1.7125641772,   5.1376925315,   5.1376925315]),  # N frac=(0.250,0.750,0.750)
    ]
    ...
    print(f"  cell:       {lattice[0][0]:.4f} bohr (= 3.6250 Å)")
